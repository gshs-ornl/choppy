...
class Choppy:
  ...
  def __init__(self, shape_archive, raster_file, output_dir = os.getcwd(),
                        statistics='min,max,mean,median,majority,sum,std,count',
    ...
    assert output_format in ['xlsx','csv','tsv','none']
    ...
    pa.extract_archive(shape_archive.name, outdir=self.working_directory.name)
    print(os.listdir(self.working_directory.name))
    shape_dir_name = os.path.basename(
        os.path.splitext(self.shape_archive.name)[0])
  ...
  def __str__(self):
    ...
    return 'Choppy class object:\n\t' + \
           'input shape archive:\t' + self.shape_archive.name + '\n\t' + \
           'input shape file:\t' + self.shape_file + '\n\t' + \
           'input raster file:\t' + self.raster_file.name + '\n\t' + \
  ...
  def chop(self):
    stats_data = zonal_stats(self.shape_file,
                    self.raster_file.name, geojson_out=self.geojson,
                    all_touched=self.all_touched, stats=self.statistics)
